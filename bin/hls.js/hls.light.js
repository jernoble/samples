  ...
  var BufferController = /*#__PURE__*/function () {
    ...
    function BufferController(_hls) {
      ...
      this._onEndStreaming = function (event) {
        console.log("ManagedMediaSource \"" + event.type + "\" event");
    ...
    _proto.onMediaAttaching = function onMediaAttaching(event, data) {
      ...
      if (media && MediaSource$1) {
        ...
        if ('buffering' in ms) {
          console.log("ManagedMediaSource buffering value of \"" + ms.buffering + "\"");
        }
        if ('quality' in ms) {
          console.log("ManagedMediaSource quality value of \"" + ms.quality + "\"");
        }
      ...
      var _loop = function _loop(trackName) {
        if (!sourceBuffer[trackName]) {
          ...
          try {
            ...
            _this9.addBufferListener(sbName, 'updatestart', _this9._onSBUpdateStart);
            _this9.addBufferListener(sbName, 'updateend', _this9._onSBUpdateEnd);
            _this9.addBufferListener(sbName, 'error', _this9._onSBUpdateError);
            // ManagedSourceBuffer bufferedchange event
            _this9.addBufferListener(sbName, 'bufferedchange', function (event) {
              console.log("ManagedSourceBuffer \"" + event.type + "\" event");
              _this9.hls.trigger(Events.BUFFER_FLUSHED, {
                type: trackName
              });
            });
            _this9.tracks[trackName] = {
              buffer: sb,
              codec: codec,
            ...
            _this9.hls.trigger(Events.ERROR, {
              type: ErrorTypes.MEDIA_ERROR,
